...
# 创建兼容的 utils.helper 模块（解决 coze_coding_utils 的导入问题）
os.makedirs(os.path.join(project_root, 'utils'), exist_ok=True)
helper_path = os.path.join(project_root, 'utils', 'helper.py')
...
utils_init_path = os.path.join(project_root, 'utils', '__init__.py')
...
# 注册数据库API路由
from api_routes import router as db_router
...
async def health_check():
    try:
        ...
        return {
            ...
            "features": [
                "chat", "tts", "asr", 
                "resume_analysis", "evaluation_report",
                "user_management", "interview_history",
                "favorite_questions"
            ]
        }
    except Exception as e:
...
async def speech_to_text(request: Request):
    ...
    try:
        ...
        audio_base64 = body.get("base64")
        
        if not audio_url and not audio_base64:
            return {"error": "url or base64 is required"}, 400
        
        ctx = new_context(method="asr.recognize")
        client = ASRClient(ctx=ctx)
        
        if audio_url:
            text, data = client.recognize(uid="interview_user", url=audio_url)
        else:
            text, data = client.recognize(uid="interview_user", base64_data=audio_base64)
...
def parse_args():
    ...
    parser.add_argument("--ssl-keyfile", type=str, default="", help="SSL key file path for HTTPS")
    parser.add_argument("--ssl-certfile", type=str, default="", help="SSL certificate file path for HTTPS")
...
def start_http_server(port, ssl_keyfile="", ssl_certfile=""):
    workers = 1
    reload = False
    ...
    if use_https:
        logger.info(f"Start HTTPS Server, Port: {port}, Workers: {workers}")
...
if __name__ == "__main__":
    args = parse_args()
    if args.m == "http":
        start_http_server(args.p, args.ssl_keyfile, args.ssl_certfile)
    elif args.m == "flow":
